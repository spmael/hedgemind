...
**Current Phase**: Early Development (Foundation Layer)

The project is currently building the foundational infrastructure for multi-tenant, organization-scoped portfolio risk analysis. Core architectural components are in place.
...
All apps are scaffolded following the recommended architecture:
...
- `apps/reference_data` - Reference data models (scaffolded)
- `apps/portfolios` - Portfolio management (scaffolded)
...
- `apps/audit` - Audit logging (scaffolded)
...
- **Test coverage**: Organization isolation tests implemented

### 🚧 In Progress / Planned

- Domain models implementation (Portfolio, Instrument, Holding, etc.)
- Portfolio ingestion (CSV/Excel upload)
- Exposure calculation engine
- Risk analytics (concentration, FX, duration)
...
2. **Parse & Normalize** (async): Job validates → maps columns → creates `Holding` records
