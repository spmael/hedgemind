"""
Management command to import instrument price observations from Excel.

    Usage:
        # Import from file (must be within organization context)
        python manage.py import_instrument_prices_excel \
            --file ./scripts/data/prices.xlsx \
            --source-code BVMAC \
            --sheet PRICES \
            --org-id 1
"""

from __future__ import annotations

import hashlib
import os
from pathlib import Path

from django.core.management.base import BaseCommand, CommandError

from apps.audit.models import AuditEvent
from apps.organizations.models import Organization
from apps.reference_data.services.prices.canonicalize import canonicalize_prices
from apps.reference_data.services.prices.import_excel import import_prices_from_file
from libs.tenant_context import organization_context


class Command(BaseCommand):
    """
    Management command for importing instrument price observations from Excel files.

    This command imports price data from Excel files. Instruments are organization-scoped,
    so the command requires an organization context.

    Key Features:
    - Imports from Excel file directly
    - Creates/updates InstrumentPriceObservation records within organization context
    - Validates required fields and choice values
    - Supports all price types: close, bid, ask, mid, NAV, etc.
    - Handles optional volume field

    Usage Example:
        python manage.py import_instrument_prices_excel \
            --file ./scripts/data/prices.xlsx \
            --source-code BVMAC \
            --sheet PRICES \
            --org-id 1

    Note:
        This command is primarily intended for reference data team operations.
        Excel format expected:
            date | instrument_id | price | price_type | quote_convention | clean_or_dirty | Volume
    """

    help = "Import instrument price observations from Excel file"

    def add_arguments(self, parser):
        """Add command-line arguments."""
        parser.add_argument(
            "--file",
            type=str,
            required=True,
            help="Path to Excel file",
        )
        parser.add_argument(
            "--source-code",
            type=str,
            required=True,
            help="Code of the MarketDataSource (e.g., 'BVMAC')",
        )
        parser.add_argument(
            "--sheet",
            type=str,
            default="PRICES",
            help="Sheet name to read (default: PRICES)",
        )
        parser.add_argument(
            "--org-id",
            type=int,
            required=True,
            help="Organization ID (required for organization-scoped instruments)",
        )
        parser.add_argument(
            "--revision",
            type=int,
            default=0,
            help="Revision number (0 = initial, 1+ = corrections)",
        )
        parser.add_argument(
            "--actor-id",
            type=int,
            help="User ID of the actor performing this action (for audit log)",
        )
        parser.add_argument(
            "--canonicalize",
            action="store_true",
            help="Run canonicalization after import",
        )

    def handle(self, *args, **options):
        """Execute the command."""
        file_path = options.get("file")
        org_id = options.get("org_id")
        source_code = options.get("source_code")
        revision = options.get("revision", 0)

        # Validate file exists
        if not os.path.exists(file_path):
            raise CommandError(f"File not found: {file_path}")

        # Validate organization exists
        try:
            organization = Organization.objects.get(id=org_id)
        except Organization.DoesNotExist:
            raise CommandError(f"Organization with id={org_id} not found")

        # Get actor if provided
        actor = None
        actor_id = options.get("actor_id")
        if actor_id:
            from django.contrib.auth import get_user_model

            User = get_user_model()
            try:
                actor = User.objects.get(pk=actor_id)
            except User.DoesNotExist:
                self.stdout.write(
                    self.style.WARNING(
                        f"User with ID {actor_id} does not exist, proceeding without actor"
                    )
                )

        self.stdout.write(f"Importing price observations from file: {file_path}")
        self.stdout.write(f"Organization: {organization.name} (ID: {org_id})")
        self.stdout.write(f"Source: {source_code}")
        self.stdout.write(f"Revision: {revision}")
        self.stdout.write(f"Sheet: {options.get('sheet', 'first sheet')}")
        self.stdout.write("")

        # Calculate file hash for audit
        file_hash = self._calculate_file_hash(file_path)

        # Import within organization context
        with organization_context(org_id):
            try:
                # Import
                result = import_prices_from_file(
                    file_path=file_path,
                    source_code=source_code,
                    sheet_name=options.get("sheet", "PRICES"),
                    revision=revision,
                )

                self.stdout.write(
                    self.style.SUCCESS(
                        f"✓ Created {result['created']} observations, "
                        f"updated {result['updated']}"
                    )
                )

                # Report errors
                if result.get("errors"):
                    self.stdout.write("")
                    self.stdout.write(
                        self.style.ERROR(f"✗ {len(result['errors'])} errors:")
                    )
                    for error in result["errors"][:20]:  # Show first 20
                        self.stdout.write(f"  - {error}")
                    if len(result["errors"]) > 20:
                        self.stdout.write(
                            f"  ... and {len(result['errors']) - 20} more"
                        )

                # Create audit event
                self._create_import_audit(
                    file_path, organization, result, file_hash, actor, source_code
                )

                # Run canonicalization if requested
                if options.get("canonicalize"):
                    self.stdout.write("")
                    self.stdout.write("Running canonicalization...")
                    canonicalize_result = canonicalize_prices()
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"✓ Canonicalized: {canonicalize_result['created']} created, "
                            f"{canonicalize_result['updated']} updated"
                        )
                    )
                    if canonicalize_result.get("errors"):
                        self.stdout.write(
                            self.style.WARNING(
                                f"⚠ {len(canonicalize_result['errors'])} canonicalization errors"
                            )
                        )

            except Exception as e:
                self.stdout.write(self.style.ERROR(f"✗ Import failed: {str(e)}"))
                raise

    def _calculate_file_hash(self, file_path: str) -> str:
        """Calculate SHA256 hash of file for audit logging."""
        sha256_hash = hashlib.sha256()
        with open(file_path, "rb") as f:
            for byte_block in iter(lambda: f.read(4096), b""):
                sha256_hash.update(byte_block)
        return sha256_hash.hexdigest()

    def _create_import_audit(
        self,
        file_path: str,
        organization: Organization,
        result: dict,
        file_hash: str,
        actor,
        source_code: str,
    ):
        """Create audit event for import."""
        file_name = Path(file_path).name

        metadata = {
            "organization_id": organization.id,
            "organization_name": organization.name,
            "file_name": file_name,
            "file_path": file_path,
            "file_hash": file_hash,
            "source_code": source_code,
            "observations_created": result.get("created", 0),
            "observations_updated": result.get("updated", 0),
            "total_rows": result.get("total_rows", 0),
            "error_count": len(result.get("errors", [])),
            "command": "import_instrument_prices_excel",
        }

        # Add error summary (first 10 errors to avoid huge JSON)
        if result.get("errors"):
            metadata["errors_summary"] = result["errors"][:10]

        AuditEvent.objects.create(
            organization_id=organization.id,
            actor=actor,
            action="MARKETDATA_IMPORT_APPLIED",
            object_type="InstrumentPriceObservation",
            object_id=None,  # Not a single object, but a collection
            object_repr=f"Imported price observations from {file_name}: {result.get('created', 0)} created, {result.get('updated', 0)} updated",
            metadata=metadata,
        )
