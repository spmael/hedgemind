"""
Admin interfaces for FX rate models.
"""
...
@admin.register(FXRateObservation)
class FXRateObservationAdmin(admin.ModelAdmin):
    """
    Admin interface for FXRateObservation model.

    Provides read-only interface for raw FX rate observations.
    """
...
class FXRateAdmin(admin.ModelAdmin):
    """
    Admin interface for FXRate model.

    Provides management interface for canonical FX rates.
    """
    ...
    search_fields = ["base_currency", "quote_currency"]
    readonly_fields = ["created_at"]
    raw_id_fields = ["chosen_source"]
    ordering = ["-date", "base_currency", "quote_currency"]


@admin.register(FXRateImport)
class FXRateImportAdmin(admin.ModelAdmin):
    """
    Admin interface for FXRateImport model.

    Provides management interface for FX rate import tracking with enhanced
    ...
    def export_errors_action(self, request, queryset):
        ...
        response["Content-Disposition"] = 'attachment; filename="fx_import_errors.csv"'
